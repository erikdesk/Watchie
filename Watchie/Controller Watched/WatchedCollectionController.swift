...
private let reuseIdentifier = "WatchedCell"

class WatchedCollectionController: UICollectionViewController {
...
//        navigationItem.rightBarButtonItem = UIBarButtonItem.init(customView: switchControl)
    ...
    func updateDisplay() {
        reloadDataAndUpdateDisplay(group: "Watched")
    }
    
    override func viewWillAppear(_ animated: Bool) {
        super.viewWillAppear(animated) // this runs before tab is clicked, not just on once app load.
        collectionView!.register(WatchedCell.self, forCellWithReuseIdentifier: WatchedCell.identifier)
        reloadDataAndUpdateDisplay(group: "Watched")
    }
                        ...
                        switch result {
                            case .success(let images):
                                ...
                                self.dataSource = .init(collectionView: self.collectionView, cellProvider: { (collectionView, indexPath, item) -> UICollectionViewCell? in
                                    let cell = collectionView.dequeueReusableCell(withReuseIdentifier: WatchedCell.identifier, for: indexPath) as! WatchedCell
                                    cell.watchedNavController = self.navigationController as? WatchedNavController
                                    cell.watchedCollectionController = self
