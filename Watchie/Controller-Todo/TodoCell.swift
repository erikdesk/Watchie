...
class TodoCell: UICollectionViewCell, UIContextMenuInteractionDelegate {
    
    static var identifier: String = "TodoCell"
    weak var todoNavController: TodoNavController!
    weak var todoCollectionController: TodoCollectionController!
    var snippet: Snippet!
...
//        topNavVC.pushViewController(detailVC, animated: true)
    ...
    func generateActionItems(todoEnabled: Bool, watchedEnabled: Bool, watchedEnabled trashedEnabled: Bool) -> [UIAction] {
        ...
        let todoActionEnabled = UIAction(title: "Add to To-do List", image: UIImage(systemName: "newspaper.fill")) { _ in
            let _ = NotificationPopupView(frame: self.frame, parentView: self.todoNavController.view, message: "\(self.snippet.title) has been added to To-do List")
            CoreDataAPI.shared.addOrReplaceToCoreData(id: self.snippet.id, group: "Todo", watchedDate: Date())
            self.todoCollectionController.updateDisplay()
        }
        ...
        let watchedActionEnabled = UIAction(title: "Mark as Watched", image: UIImage(systemName: "play.tv.fill")) { _ in
            let _ = NotificationPopupView(frame: self.frame, parentView: self.todoNavController.view, message: "\(self.snippet.title) has been marked as Watched")
        ...
        let trashedActionEnabled = UIAction(title: "Send to Trash Bin", image: UIImage(systemName: "trash.fill")) { _ in
            let _ = NotificationPopupView(frame: self.frame, parentView: self.todoNavController.view, message: "\(self.snippet.title) has been sent to the Trash Bin")
